...
function doubleCheck (arr) {
  for (i=0; i<arr.length; i++) {if (arr[i]= "chocolate"){return arr} else {arr.push("chocolate")};
  }
return arr; 
}; 
...
function looper (arr){
  ...
  for (i=0; i<arr.length; i++){
    if(arr[i]%2 != 0){ mySum+= arr[i]}
    else if (arr[i] >= 100){mySum+= arr[i]}
  }; 
  return mySum; 
  }; 
...
function outerFn (){
  return function (){return "Aftab Baig"}
